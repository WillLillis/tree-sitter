...
rule_tests! {
    ...
    repeat1 {
        ...
        ExpectedRule::repeat(ExpectedRule::String("x".into()))
    }
    optional_combinator {
        r#"grammar { language: "test" } rule program { optional("x") }"#,
        ExpectedRule::choice(vec![ExpectedRule::String("x".into()), ExpectedRule::Blank])
    }
    blank_combinator {
    ...
    token_combinator {
        ...
        ExpectedRule::token(ExpectedRule::seq(vec![ExpectedRule::String("a".into()), ExpectedRule::String("b".into())]))
    }
    token_immediate_combinator {
        r#"grammar { language: "test" } rule program { token_immediate("x") }"#,
        ExpectedRule::immediate_token(ExpectedRule::String("x".into()))
    }
    field_combinator {
        r#"grammar { language: "test" }
        rule program { field(name, "x") }
        rule name { "n" }"#,
        ExpectedRule::field("name".to_string(), ExpectedRule::String("x".into()))
    }
    alias_with_string {
        r#"grammar { language: "test" }
        rule program { alias(identifier, "id") }
        rule identifier { regexp("[a-z]+") }"#,
        ExpectedRule::alias(ExpectedRule::NamedSymbol("identifier".into()), "id".to_string(), false)
    }
    alias_with_named_rule {
        ...
        rule _impl { "impl" }
        rule block { "{}" }"#,
        ExpectedRule::alias(ExpectedRule::NamedSymbol("_impl".into()), "block".to_string(), true)
    }
    alias_with_variable_target {
        ...
        rule some_rule { "y" }"#,
        ExpectedRule::alias(ExpectedRule::String("x".into()), "some_rule".to_string(), true)
    }
    alias_with_let_string_target {
        ...
        rule identifier { regexp("[a-z]+") }"#,
        ExpectedRule::alias(ExpectedRule::NamedSymbol("identifier".into()), "renamed".to_string(), false)
    }
    ...
    prec_left {
        ...
        ExpectedRule::prec_left(Precedence::Integer(2), ExpectedRule::String("x".into()))
    }
    prec_right {
        r#"grammar { language: "test" } rule program { prec_right(3, "x") }"#,
        ExpectedRule::prec_right(Precedence::Integer(3), ExpectedRule::String("x".into()))
    }
    prec_dynamic {
        r#"grammar { language: "test" } rule program { prec_dynamic(4, "x") }"#,
        ExpectedRule::prec_dynamic(4, ExpectedRule::String("x".into()))
    }
    prec_dynamic_negative {
        r#"grammar { language: "test" } rule program { prec_dynamic(-1, "x") }"#,
        ExpectedRule::prec_dynamic(-1, ExpectedRule::String("x".into()))
    }
    prec_with_string_name {
        r#"grammar { language: "test" } rule program { prec_left("assign", "x") }"#,
        ExpectedRule::prec_left(Precedence::Name("assign".into()), ExpectedRule::String("x".into()))
    }
    rule_reference_in_body {
        r#"grammar { language: "test" }
        rule program { other }
        rule other { "x" }"#,
        ExpectedRule::NamedSymbol("other".into())
    }
    for_inline_in_seq {
        r#"grammar { language: "test" }
        rule program {
        ...
        ExpectedRule::seq(vec![
            ExpectedRule::String("start".into()),
            ExpectedRule::String("a".into()),
            ExpectedRule::String("b".into()),
            ExpectedRule::String("end".into()),
        ])
    }
    raw_ident_as_rule_reference {
        ...
        rule program { r#let }
        rule r#let { "in" }"#,
        ExpectedRule::NamedSymbol("let".into())
    }
    string_hex_escape {
    ...
    string_unicode_escape_braced {
        ...
        ExpectedRule::String("\u{1F389}".into())
    }
    string_unicode_escape_max {
        ...
        ExpectedRule::String("\u{10FFFF}".into())
    }
    ...
    int_arith_named_plus_literal {
        ...
        ExpectedRule::prec(Precedence::Integer(6), ExpectedRule::String("x".into()))
    }
    int_arith_two_named {
    ...
    int_arith_in_let_binding {
        ...
        ExpectedRule::prec(Precedence::Integer(11), ExpectedRule::String("x".into()))
    }
    ...
    raw_ident_keyword_collisions {
        ...
        rule r#alias { "a" }"#,
        ExpectedRule::seq(vec![
            ExpectedRule::NamedSymbol("field".into()),
            ExpectedRule::NamedSymbol("import".into()),
            ExpectedRule::NamedSymbol("alias".into()),
        ])
    }
    regexp_combinator {
        r#"grammar { language: "test" } rule program { regexp("[a-z]+") }"#,
        ExpectedRule::Pattern("[a-z]+".into(), String::new())
    }
    regexp_with_flags {
        r#"grammar { language: "test" } rule program { regexp("[a-z]+", "i") }"#,
        ExpectedRule::Pattern("[a-z]+".into(), "i".into())
    }
    concat_combinator {
...
fn prec_inside_token_immediate() {
    ...
    assert_rule(
        &g.pool,
        g.variables[0].root,
        &ExpectedRule::immediate_token(ExpectedRule::prec(
            Precedence::Integer(1),
            ExpectedRule::pattern("[a-z]+", ""),
        )),
    );
}
...
fn reserved_combinator() {
    ...
    assert_rule(
        ...
        &ExpectedRule::Reserved {
            ...
            rule: Box::new(ExpectedRule::NamedSymbol("identifier".into())),
        },
    );
}
...
fn reserved_inherited() {
    ...
    assert_rules(
        ...
        &[
            ExpectedRule::String("get".into()),
            ExpectedRule::String("set".into()),
        ],
    );
}
