...
/// deep tests.
fn deep_token_chain(n: usize) -> (SharedAst, ModuleContext, RulePool) {
...
fn typecheck_deep_nesting_does_not_overflow() {
    ...
    // walk handles it.
    let (shared, ctx, pool) = deep_token_chain(50_000);
    let mut env = TypeEnv::default();
    typecheck::check(&shared, &ctx, &mut env, pool.strs()).unwrap();
}
