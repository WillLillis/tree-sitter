...
fn check_token_conflicts(
    ...
    // Vectorized check: does terminal i conflict with or match-prefix any
    // terminal in the set? O(n/64) word-level ANDs instead of O(n) iteration.
    let conflict_row =
        &token_conflict_map.conflict_or_prefix_bits[row_start..row_start + wpr];
    ...
    // Vectorized check: does terminal i overlap (in either direction) with any
    // non-coincident terminal in the set?
